...
void step4_update(int* n_sols, int* Sols, int* iscrit, int* combs);
...
{ 
    ...
    for (card = 1; card <=kmax ;card++)
    {
        ...
        while (n_analysed_combs < Cn[n_MU * (n_colums_Cn) + card])
        {
            ...
            //Prop. 2
            step3_confirmation(is_crit, combs, Sols, n_sols);
            //Prop. 1
            step2_evaluation(is_crit, E, combs, UMs, meas_plan);
            //Atualização
            step4_update(&n_sols, Sols, is_crit, combs);
    ...
    for (int i = 0; i < n_combs_in_wave; i++)
    {
        if (iscrit[i] == 1)
        {
            for (int j = 0; j < kmax; j++)
                Sols[*n_sols *kmax + j] = combs[i * kmax + j];
            *n_sols += 1;
        }
    }
}
